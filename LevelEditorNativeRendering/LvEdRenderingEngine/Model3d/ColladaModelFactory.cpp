...
void ColladaModelFactory::ProcessEffect(Model3dBuilder * builder, xml_node* node)
{
	...
	mat->texNames[TextureType::DIFFUSE] = PerformSmartLookupForTextureName(builder, GetTextureFileFromSampler(builder, samplerName));
    ...
    if(sampName)
        mat->texNames[TextureType::NORMAL] = PerformSmartLookupForTextureName(builder, GetTextureFileFromSampler(builder, sampName));
    ...
    {		
        ...
        {            
            mat->texNames[TextureType::DIFFUSE] = PerformSmartLookupForTextureName(builder, GetTextureFileFromSampler(builder, std::string(symbolValue)));
        }
        if(strcmp(symbolName, "NormalSampler")==0)
        {
            mat->texNames[TextureType::NORMAL] = PerformSmartLookupForTextureName(builder, GetTextureFileFromSampler(builder, std::string(symbolValue)));
        }
    }
...
// ------------------------------------------------------------------------------------------------
const std::string ColladaModelFactory::PerformSmartLookupForTextureName(Model3dBuilder * builder, const std::string& filepath)
{
	const std::wstring sourceFilename = builder->m_model->GetSourceFileName();
	std::vector<wchar_t> buf(sourceFilename.begin(), sourceFilename.end());
	buf.push_back(0);

	PathRemoveFileSpec(buf.data());

	std::wstring sourceDirectory(buf.data());

	std::wstring textureInDirectory = (sourceDirectory + std::wstring(L"\\") + std::wstring(filepath.begin(), filepath.end()));
	std::wstring textureInParentDirectory = (sourceDirectory + std::wstring(L"\\..\\") + std::wstring(filepath.begin(), filepath.end()));
	std::wstring textureInParentTextureDirectory = (sourceDirectory + std::wstring(L"\\..\\texture\\") + std::wstring(filepath.begin(), filepath.end()));
	std::wstring relativeTextureInDirectory = (std::wstring(filepath.begin(), filepath.end()));
	std::wstring relativeTextureInParentDirectory = (std::wstring(L"..\\") + std::wstring(filepath.begin(), filepath.end()));
	std::wstring relativeTextureInParentTextureDirectory = (std::wstring(L"..\\texture\\") + std::wstring(filepath.begin(), filepath.end()));

	if (PathFileExists(textureInDirectory.c_str()))
	{
		return std::string(relativeTextureInDirectory.begin(), relativeTextureInDirectory.end());
	}

	if (PathFileExists(textureInParentDirectory.c_str()))
	{
		return std::string(relativeTextureInParentDirectory.begin(), relativeTextureInParentDirectory.end());
	}

	if (PathFileExists(textureInParentTextureDirectory.c_str()))
	{
		return std::string(relativeTextureInParentTextureDirectory.begin(), relativeTextureInParentTextureDirectory.end());
	}
