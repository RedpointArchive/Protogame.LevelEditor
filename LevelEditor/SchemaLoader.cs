...
namespace LevelEditor
{
    ...
    public class SchemaLoader : XmlSchemaTypeLoader, ISchemaLoader
    {
        ...
        public SchemaLoader(IGameEngineProxy gameEngine)
        {
            ...
            SchemaResolver = new ResourceStreamResolver(Assembly.GetExecutingAssembly(), "LevelEditor._GameBundle.Generated");
            Load("_tempbundle.xsd");           
        }
        ...
        protected override void OnSchemaSetLoaded(XmlSchemaSet schemaSet)
        {
            ...
            // Add component editor to objects that support component editing.
            foreach (var nestedType in typeof (Schema).GetNestedTypes())
            {
                var fields = nestedType.GetFields(BindingFlags.Static | BindingFlags.Public);
                if (fields.Any(x => x.Name == "componentChild") && fields.Any(x => x.Name == "Type"))
                {
                    // This object support components.
                    var typeField = fields.First(x => x.Name == "Type");
                    var componentField = fields.First(x => x.Name == "componentChild");
                    ((DomNodeType) typeField.GetValue(null)).SetTag(new PropertyDescriptorCollection(
                        new PropertyDescriptor[]
                        {
                            new ChildPropertyDescriptor(
                                "Components".Localize(),
                                (ChildInfo)componentField.GetValue(null),
                                null,
            ...
            IDictionary<NamedMetadata, IList<XmlNode>> annotations)
        {
            ...
            {                               
                ...
                {
                    if (xmlnode.LocalName == "Protogame.Info")
                    {
                        nodeType.SetTag(
                            "QualifiedName", 
                            xmlnode.Attributes?.OfType<XmlAttribute>().Where(x => x.LocalName == "QualifiedName").Select(x => x.Value).FirstOrDefault());
                    }
