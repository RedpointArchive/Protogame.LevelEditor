...
namespace Sce.Atf
{
    ...
    public class ResourceStreamResolver : XmlUrlResolver
    {
        ...
        public override object GetEntity(Uri absoluteUri, string role, Type returnType)
        {
            ...
            {
                if (absoluteUri.AbsolutePath.EndsWith("gap.xsd"))
                {
                    return m_assembly.GetManifestResourceStream("LevelEditor._GameBundle.Provided.gap.xsd");
                }
                if (absoluteUri.AbsolutePath.EndsWith("level_editor.xsd"))
                {
                    return m_assembly.GetManifestResourceStream("LevelEditor._GameBundle.Provided.level_editor.xsd");
                }
                if (absoluteUri.AbsolutePath.Contains("/bundle."))
                {
                    return m_assembly.GetManifestResourceStream("LevelEditor._GameBundle." + absoluteUri.AbsolutePath.Split('/').Last());
                }
